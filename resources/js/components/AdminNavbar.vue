...
<!--   main-header  custom_main_header-->
    <nav :class=" main_header_collapsed ? 'main-header-collapsed' : 'main-header' " class="navbar navbar-expand navbar-dark" id="nav_main_header">
        ...
        <ul class="navbar-nav ml-auto">
            <!-- Navbar Search -->
            <li class="nav-item">
                <i class="test-device p-1 m-1" v-if="app_version == 'D'">
                       <span class="hidden_info">
                          {{ app_version }}
                       </span>
                </i>
            </li>
...
<!--                    Route::get('user/notifications', [UserNotificationsController::class, 'index'])->name('notifications.index');-->
            ...
            <li class="nav-item" v-show="$page.props.auth">
                <inertia-link :href="route('user.profile.index')" class=" m-0 p-1" title="Your profile">
                     <i :class="getHeaderIcon('profile')" class="action_icon icon_right_text_margin"></i>
                </inertia-link>
...
export default {
    ...
    setup() {
        let main_header_collapsed = ref(false)
