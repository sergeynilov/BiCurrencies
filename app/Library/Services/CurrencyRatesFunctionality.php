...
//            ->distinct('ads.id')
        ...
        $userCurrencySubscriptions = UserCurrencySubscription
            ::leftJoin(with(new User)->getTable(), with(new User)->getTable() . '.id', '=',
                with(new UserCurrencySubscription)->getTable() . '.user_id')
            ->leftJoin(with(new Currency)->getTable(), with(new Currency)->getTable() . '.id', '=',
                with(new UserCurrencySubscription)->getTable() . '.currency_id')
            ->where(with(new Currency)->getTable() . '.active', true)
            ->where(with(new User)->getTable() . '.status', 'A')
            ->select(
                with(new Currency)->getTable() . '.*',
                with(new User)->getTable() . '.id as user_id',
            )
//            ->where(with(new UserCurrencySubscription)->getTable() . '.id', 2) // DEBUGGING - NOT BASE CURRENCY
            ->orderBy('user_id', 'asc')
            ->get();
//            ->pluck('currency_id')
//            ->toArray();
        ...
        foreach ($userCurrencySubscriptions as $nextUserCurrencySubscription) {
            if ($is_first) {
                ...
                if (count($userCurrencySubscriptions) === 1) {
                    ...
                    $nextUserCurrencies[] = $nextUserCurrencySubscription->id; // currency_id
...
//            \Log::info(varDump($nextCurrency, ' -1 $nextCurrency::'));
            if ( ! empty($nextCurrency) and $nextCurrency['active']) { // In report only active currencies
                \Log::info(varDump($nextCurrency['latest_currency_history'],
                    ' -1 $nextCurrency->latest_currency_history::'));
    ...
    private static function writeRatesIntoDb($operation_date, $rates)
    {
        try {
            ...
            foreach ($rates as $currencyRateRowCharCode => $currencyRateRowValue) { // all currency rate rows
                ...
                $currencyHistory->value       = str_replace(',', '.',
                    (string)$currencyRateRowValue);   // value: '55,9618' fo
